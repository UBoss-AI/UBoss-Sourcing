...
| **Object storage** | `S3_ENDPOINT`, `S3_BUCKET`, `S3_REGION`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`, `S3_FORCE_PATH_STYLE`, `STORAGE_PUBLIC_BASE_URL`, `STORAGE_LOCAL_DIR` | keys | Required when `STORAGE_DRIVER=s3`. `S3_REGION=auto` for Cloudflare R2; `S3_FORCE_PATH_STYLE=true` for R2 and MinIO. `STORAGE_PUBLIC_BASE_URL` is the CDN or bucket base for `products/` only — an absolute URL, or a root-relative path such as `/media` when a proxy serves the files on the same origin as the page |
...
| **Admin MFA** | `FEATURE_ADMIN_MFA` | no | Defaults on and production refuses to start if disabled. Every fresh admin session must pass TOTP or a single-use recovery code. |
| **Rate limits and lockout** | `RATE_LIMIT_GLOBAL_PER_MINUTE` (300), `RATE_LIMIT_LOGIN_PER_15MIN` (10), `LOGIN_LOCKOUT_THRESHOLD`, `LOGIN_LOCKOUT_MINUTES` | no | Counters and account lockout are shared in MariaDB across all API instances. |
...
| SAST | CodeQL (`javascript-typescript`, `security-extended`) | **mandatory workflow** | Runs on pull requests, pushes to `main`, and weekly. A private repository must enable GitHub Advanced Security; a missing licence/configuration fails visibly instead of skipping analysis. |
...
| ~~S3~~ | ~~CSP is Report-Only~~ | **Fixed** | The shipped nginx header is enforcing. Retest Stripe and Razorpay checkout in staging after editing any allowed origin. |
...
| ~~S5~~ | ~~Application rate limit is per process~~ | **Fixed** | `DatabaseRateLimitStore` uses the `RateLimitBucket` table, so all API instances increment one atomic window. Database failure fails closed. |
...
| ~~S7~~ | ~~No mandatory MFA on staff accounts~~ | **Fixed** | Every admin route is blocked until the account enrols TOTP and the current session challenges it. Recovery codes are hashed and single-use; production refuses `FEATURE_ADMIN_MFA=false`. |
...
| ~~S9~~ | ~~**No dependency or secret scanning in CI**, because there is no CI~~ | **Fixed in the repository** | `npm audit --audit-level=high`, CycloneDX SBOMs and Dependabot cover all five packages. Gitleaks 8.30.1 is pinned, checksum-verified and scans full history. CodeQL is mandatory. **Outstanding live setting:** enable GitHub Advanced Security and push protection. |
...
| ~~B11~~ | ~~Enforce the CSP (§11.4)~~ | **DONE in nginx.** The web policy now ships as enforcing rather than report-only. Complete a 3-D Secure and Razorpay staging checkout before traffic cutover. | Tech owner — staging validation |
...
| ~~B14~~ | ~~Set up CI/CD (§15)~~ | **DONE in the repository:** mandatory CodeQL, immutable action SHAs, checksum-verified Gitleaks, audits/SBOMs for all five packages, and a mandatory deployment smoke URL. **Still yours:** GitHub Advanced Security, branch/environment protection, push protection, restricted deploy key and public URLs. | Tech owner — **repository settings outstanding** |
