...
/** Where a consignment stands, in the buyer's terms. Mirrors the server. */
...
export interface OrderShipment {
  /**
   * Present only on a consignment (not on a seller's own dispatch note), and
   * optional so a response from an older server still renders.
   */
  deliveryStage?: CustomerDeliveryStage;
  /** The carrier's OWN waybill number. Never generated by this system. */
  carrierTrackingNumber?: string | null;
  /** What the buyer is told, in order. Never the carrier's internal notes. */
  events?: { status: string; description: string | null; occurredAt: string }[];
  /**
