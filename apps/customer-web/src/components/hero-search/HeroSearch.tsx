 ...
 *
 * One large search bar, with a row of destinations above it. It replaced the
 * pair of call-to-action buttons that used to sit here, and the swap is the
 * point: "Browse the catalogue" asked somebody to go and look for what they
 * wanted, and a search box lets them say it.
 *
 * The row looks like tabs and is not tabs. **Every item in it is a link to a
 * page**, and the one that is underlined is the page you are on — which, on
 * the greeting page, is Home:
 *
 *   - **Home** is the greeting page itself, `/`. It is the current item while
 *     you are here, and pressing it from anywhere else brings you back. It
 *     leads the row because it is the page the other two are reached from, and
 *     because a row whose first item is a destination you are already at is
 *     how the rest of the storefront's breadcrumbs read.
 *   - **Products** is the catalogue, `/products`, and pressing it goes
 *     straight there — the browse-all page with its filters, facets,
 *     pagination and category structure. It used to be marked as the current
 *     item on this page, which said the greeting page *was* the catalogue; it
 *     is not, and somebody who wanted the catalogue had no way to press for
 *     it. The bar underneath still searches the catalogue whichever item is
 *     current: submitting goes to `/products?q=`, and submitting an empty box
 *     goes to the same page with no term, which is the whole catalogue.
 *     Nothing here renders a result — this module hands off, and the catalogue
 *     page owns what a result looks like.
 *   - **AI Assistant** is a **link to another page**, and pressing it goes
 *     straight there. It used to switch the bar into an "ask a question" mode
 *     and navigate on submit, which meant two presses to reach a page that is
 ...
 *
 *     It is labelled "AI Assistant" because that is what the sourcing graphic
 *     beside it calls the same destination, and two names for one page on one
 *     screen is one of them a visitor has to rule out first. The page is still
 ...
 * between them; every item here leaves the page. Announcing navigation as tab
 * selection is the kind of ARIA that makes a screen reader describe a control
 * that no longer exists. What is left is a labelled row of links, with the one
 * you are on marked `aria-current="page"` — which is what that value means,
 * and it stays a link so that it is never a dead word in the middle of a row
 * of live ones.
 *
 * **Which item is current comes from the URL**, not from an assumption about
 * where this module is mounted. It lives on the greeting page today, so Home
 * is what is underlined; deciding it from `useLocation` is what stops the row
 * from lying the day it is put anywhere else.
 *
 * The AI item is absent, rather than disabled, on a deployment with no AI
 * provider configured — the same rule the rest of the storefront follows for a
 ...
 * slides.** It used to be the latter, because the selection moved between
 * items of different widths in eight languages and arithmetic could not place
 * it. As a `span` inside the item it is exact in every language for free, and
 * it takes a `ResizeObserver`, a `document.fonts.ready` handler and a layout
 * effect out of the module.
 *
 * The item that is not current is full-strength ink, not grey. Each is a real
 * place a visitor is being offered, and a greyed one reads as unavailable
 * rather than as not-current — the underline and the brand colour are what say
...
function ModeLink({
  ...
  return (
    <Link
      to={to}
      onClick={onClick}
      // `page`, not `true`: this is a link to the page being displayed, which
      // is the one value of `aria-current` that says so.
      aria-current={isCurrent ? 'page' : undefined}
      className={cx(ITEM_CLASS, isCurrent ? 'text-brand' : 'text-ink hover:text-brand')}
    >
      {children}
      {isCurrent && (
        <span
...
/**
 * A hairline between two items, not a gap doing the same job at a distance.
 *
 * With items of very different widths — and eight languages' worth of each —
 * space alone stops reading as a separation and starts reading as an accident.
 */
...
function SearchModes({
  ...
  return (
    <nav
      // The row's name lists what is in it, so it has a second wording for the
      // deployment where the AI item is not. A landmark that names a
      // destination a screen reader will not find in it is worse than a vaguer
      // one.
      aria-label={hasAi ? t('heroSearch.modesLabel') : t('heroSearch.modesLabelNoAi')}
      // The bottom margin is what keeps the current item's underline OFF the
      // top edge of the search field below it. With no gap the two rules meet
    ...
    >
      ...
      {/* The AI item keeps the middle, which is where it has been since the row
          was two items. Its `onClick` is what parks whatever is in the box. */}
      {hasAi && (
        <>
          <ModeLink to={AI_MODE_PATH} isCurrent={false} onClick={onLeaveForAi}>
            {t('heroSearch.aiMode')}
