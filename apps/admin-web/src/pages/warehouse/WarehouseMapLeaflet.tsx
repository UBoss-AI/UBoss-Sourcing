/**
 * The warehouse map, drawn with Leaflet.
 *
 ...
 * covers two of the three providers: raster tiles from whatever XYZ service
 * the operator pointed at, and no background at all.
 *
 * Leaflet is loaded by dynamic `import()` inside the effect below rather than
 ...
 * that is low on stock can be a different colour from one that is fine. What
 * one looks like lives in `warehouse-marker.ts`, shared with the Google
 * implementation so the two maps cannot drift apart.
 *
...
import type { MapTiles, Warehouse } from '@/lib/warehouses';
import { MARKER_BOX, markerHtml } from './warehouse-marker';

interface WarehouseMapLeafletProps {
  warehouses: Warehouse[];
  /** Null for "no background", which is a working state rather than a fault. */
  tiles: MapTiles | null;
...
/**
 * Escaped by hand, because Leaflet's attribution control takes markup and
 * writes it with `innerHTML`.
 *
 * The one remaining place in this file where a configured string reaches the
 * DOM as HTML: `MAP_TILE_ATTRIBUTION` is free text an operator sets, and it is
 * printed in the corner of the map. Escaping it costs a licence its link and
 * keeps the text, which is the right way round for a value that arrives from a
 * settings file.
 *
 * The markers need none of this any more. They are built with `textContent` in
 * `warehouse-marker.ts`, so a warehouse called `Pune <b>2</b>` reads as its own
 * name without anything here having to stay correct.
 */
...
export function WarehouseMapLeaflet({
  warehouses,
  tiles,
  selectedId,
  onSelect,
}: WarehouseMapLeafletProps): React.JSX.Element {
  const { t } = useI18n();
    ...
    void (async () => {
      ...
      for (const warehouse of placed) {
        const marker = L.marker([warehouse.latitude, warehouse.longitude], {
          icon: L.divIcon({
            // No class of Leaflet's own. `divIcon` otherwise brings
            // `leaflet-div-icon`, which is a white box with a border - drawn
            // behind a circle that is already carrying the panel's palette.
            className: '',
            html: markerHtml(warehouse, warehouse.id === selectedId),
            // Anchored at the centre of the box rather than the bottom of it:
            // the marker is a ring centred on the coordinate, not a pin whose
