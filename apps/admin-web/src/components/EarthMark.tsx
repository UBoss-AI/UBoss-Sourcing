 ...
 * The brand mark.
 *
 * A small earth, turning slowly, at the top of the console rail. It is the same
 * object the storefront opens its landing page with —
 * `components/ui/3d-globe.tsx` — at a fraction of the size, and the same mark
 * the storefront's own header carries, which is the point: one product, one
 * mark, whichever of the three applications somebody is looking at.
 *
 ...
 *
 *   - **A logo may not depend on WebGL.** The rail is on every page, and a
 *     blocklisted driver, a locked-down browser or a machine out of GPU
 *     contexts must produce a finished rail, not a hole in one.
 ...
 *
 * It draws a 28px square: at the capped pixel ratio that is under two thousand
 * fragments a frame, which is less work than one row of a table does on a
 * hover. It takes the 1024×512 earth rather than the landing page's 2048×1024
 * — the texture is uploaded to the GPU at its own size whatever size it renders
 * at — it carries no pins, no lanes and no orbits, and its loop stops the
 * moment the tab is hidden.
 *
 * Copied from `apps/customer-web/src/components/EarthMark.tsx`, which is where
 * it started and where a change to it belongs first. The three applications are
 * three builds with no shared package; `components/ui/3d-globe.tsx` beneath it
 * is duplicated the same way and for the same reason.
 */
