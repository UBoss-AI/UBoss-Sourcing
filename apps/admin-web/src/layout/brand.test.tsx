 ...
 * "Admin console". It is Glovia now, over `Powered by UBOSS` — the product,
 * and who makes it. Both strings live in `lib/brand.ts` and nowhere else, so
...
describe('the console’s brand lockup', () => {
  ...
  it('spells both of them the one way', () => {
    ...
    // the markup says `Powered by UBOSS`, never `POWERED BY UBOSS`.
  ...
  it('is a link home, named with both lines', () => {
    ...
    const home = screen.getByRole('link', { name: `${PRODUCT_BRAND} — ${PARENT_ATTRIBUTION}` });
