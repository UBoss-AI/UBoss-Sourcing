/**
 * The console's live connection to the preorder chat inbox.
 *
 * A copy of the storefront's `lib/chat-socket.ts` with the staff path: the two
 * apps are separate packages and share no source.
 *
...
/** The tab's one staff chat socket. */
export function staffChatSocket(): ChatSocket {
  shared ??= new ChatSocket('/admin/preorder-chats/socket');
  return shared;
}
