 ...
 * factor. Drawing it in this process is what keeps the secret out of a chart
 * service, out of an HTTP response body, and out of every cache in between.
 * The component that renders it is `components/QrCode.tsx`.
 *
 * It is a copy of the logistics portal's `lib/qr.ts`, character for character
 * below this header, because the three apps share no package. The unit test
 * beside it is copied with it: two copies of an encoder with one copy of the
 * test is one app silently drawing a code nobody checks.
 *
 ...
 * at an angle, by whatever phone the administrator has in their hand, and 15%
 * recovery against 7% is the difference between one attempt and four.
 */
