...
/**
 * Every key in the catalogue.
 *
 * The other two applications widen this with the BASE of each plural family -
 * `products.count_one` and `products.count_other` are in their JSON, and what
 * a screen writes is `t('products.count', { count })`, which is not itself a
 * key. This catalogue has no plural family yet, so deriving that base here
 * would add nothing and only obscure the type. Add the derivation back
 * alongside the first `_one`/`_other` pair; until then, every key a screen may
 * pass is a key in the file, which is the stricter and more useful promise.
 */
