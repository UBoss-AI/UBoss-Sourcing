...
export function SessionProvider({ children }: { children: ReactNode }): React.JSX.Element {
  ...
  const load = useCallback(async (): Promise<void> => {
    try {
       ...
       * a deactivated company - is also a portal this person cannot use, and
       * the sign-in screen carries the message the server sent. Distinguishing
       * them here would mean a half-open portal with no session behind it.
       */
      setSession(null);
      setStage('SIGNED_OUT');
    }
  ...
  }, [stage, session, signIn, signOut, load]);
