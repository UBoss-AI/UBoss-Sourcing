 ...
 * The API client.
 *
 * Every request in the logistics portal goes through here, so the rules the
 * backend enforces are honoured in exactly one place:
 *
 ...
 *   - **Double-submit CSRF.** The `uboss_logi_csrf` cookie is the one cookie
...
/*
 * Scoped to this surface, and it is a THIRD scope rather than a second.
 *
 * A cookie is identified by name, domain and path - not by port - so all three
 * applications share one jar whenever they sit on the same hostname, which is
 * every local setup. Shared names would mean a dispatcher signing into the
 * portal silently signing a member of staff out of the console on the same
 * machine.
 *
 * The name matches `cookieNamesFor('LOGISTICS')` in the backend's auth
 * plugin. Change both or neither.
 */
const CSRF_COOKIE = 'uboss_logi_csrf';
...
async function refreshSession(): Promise<boolean> {
  refreshInFlight ??= (async (): Promise<boolean> => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/logistics/auth/refresh`, {
        method: 'POST',
        credentials: 'include',
 ...
 * arrive before showing the first. The insights panel needs the raw
 ...
 * `fetch` would quietly opt out of all three — and a streaming endpoint behind
 * a sign-in is exactly where a missed refresh looks like a dropped connection
 * instead of an expired session.
 *
 * Returns the response whatever its status. A streaming caller has to see the
 * 401 itself: it may have a half-typed question to keep hold of, which no
...
export async function requestStream(
  ...
  // The same one-refresh-at-a-time path every other request uses, so a stream
...
export const api = {
  ...
  upload: <T>(
    ...
    formData: FormData,
    options?: Omit<RequestOptions, 'method' | 'body'>,
  ): Promise<T> => request<T>(path, { ...options, method: 'POST', formData }),
};
