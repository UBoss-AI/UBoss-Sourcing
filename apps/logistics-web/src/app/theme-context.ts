 ...
 * instruction to keep following the operating system, so a member of staff
 * whose laptop turns dark at sunset gets a dark panel at sunset without ever
 * having visited this setting. `light` and `dark` are a visitor overruling
 * that machine-wide setting for this panel alone, which is a thing people do
 * — a bright warehouse office at midday, a night shift on the picking floor
 * — and which no amount of clever detection can infer.
 *
