 ...
 * A QR image fetched from a chart service would send it to whoever serves that
 * service; one generated on the server would put it in an HTTP response body
 * and in every cache between there and the browser. Neither is acceptable at
 * any price, so the code is drawn from the string, in this process, and the
...
export function QrCode({ value, size = 168 }: { value: string; size?: number }): React.JSX.Element {
  ...
  if (modules === null) {
    // The payload could not be encoded. The secret is printed beside this in
    // plain text, so the person can still type it in - which is why this fails
    // quietly rather than throwing the whole screen away.
  ...
  return (
    <svg
      ...
      width={size}
      height={size}
      // Decorative: the secret is printed beside it as text, which is the
