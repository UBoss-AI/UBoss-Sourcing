...
import { buildChatContext, chatContextInputSchema, type ChatContextInput } from './context.service.js';
import {
...
type MessageType = 'TEXT' | 'ATTACHMENT' | 'SYSTEM_EVENT' | 'STRUCTURED_OFFER';
...
export function senderKeyFor(senderType: SenderType, id: string): string {
  return `${senderType === 'CUSTOMER' ? 'C' : senderType === 'ADMIN' ? 'A' : 'S'}:${id}`;
}
...
export async function startOrContinueCustomerChat(
  ...
  if (existing !== null) {
    return continueCustomerChat(actor, existing.id, input);
  }
  ...
  try {
    messageId = await prisma.$transaction(async (tx) => {
      ...
      // The customer has read their own message.
      await markReadInTx(tx, conversationId, 'CUSTOMER', actor.userId, 1);
    ...
    if (isUniqueViolation(error, 'uq_preorder_chat_active')) {
      ...
      if (winner !== null) return continueCustomerChat(actor, winner.id, input);
    }
...
export async function customerUnreadTotal(actor: CustomerActor): Promise<number> {
...
async function markReadInTx(
  ...
  const otherSeen = await tx.preorderChatMessage.count({
    where: {
      ...
      senderType: side === 'CUSTOMER' ? { in: ['ADMIN', 'SYSTEM'] } : 'CUSTOMER',
    },
  });
...
async function stampDelivered(
  ...
  await tx.preorderChatMessage.updateMany({
    where: {
      ...
      deliveredAt: null,
      senderType: readerSide === 'CUSTOMER' ? { in: ['ADMIN', 'SYSTEM'] } : 'CUSTOMER',
    },
