...
export async function getStorefrontConfig(): Promise<Record<string, unknown>> {
  ...
  return {
    ...
    marketplace: {
      ...
      // The operator's team - who answers a preorder chat and manages a
      // delivery level: `OPERATOR_TEAM_NAME`, or the marketplace's own name.
      teamName: env.OPERATOR_TEAM_NAME.length > 0 ? env.OPERATOR_TEAM_NAME : marketplaceNameFrom(profile?.displayName),
    },
    business: {
